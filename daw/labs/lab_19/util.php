<?php
  ...
  function getEmpleados(){
    ...
    while($row = $res->fetch_array(MYSQLI_BOTH)){
      ...
      <div class="row">
        ...
        <div class="col s2">
        <a href="editar.php" class="btn-floating disabled"><i class="material-icons right">edit</i></a>
          <a href="eliminar.php" class="btn-floating disabled"><i class="material-icons right">delete_forever</i></a>    
        </div>
      </div>';
    }
    mysqli_free_result($res);
  ...
  function nuevaEntrada(){
    ...
    guardarEntrada($nombre, $rol, $departamento, $deporte);
  }

  function guardarEntrada($nombre, $rol, $departamento, $deporte){
    $db = conectDb();
    $query = "INSERT INTO usuarios (`nombre`, `rol`, `departamento`, `deporte`) VALUES (?,?,?,?)";
    if(!($statement = $db->prepare($query))){
      die("Preparación fallida: (".$db->errno.") ".$db->error);
    }
    if(!($statement->bind_param("ssss", $nombre, $rol, $departamento, $deporte))){
